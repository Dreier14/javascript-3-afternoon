...
//Code Here
let evenNumbers = mixedNumbers.filter(function(val){
  return val % 2 === 0
});
...
//Code Here
let totalPopulation = populations.reduce((accumulator, val) => {
...
let ordersTotal = orders.map((val) => {
...
let bobsTotal = purchases.reduce((accumulator,val) => {
  if (val.owner === 'Bob'){
